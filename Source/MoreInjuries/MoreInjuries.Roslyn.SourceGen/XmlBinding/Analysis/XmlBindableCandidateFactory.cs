...
internal static class XmlBindableCandidateFactory
{
    public static XmlBindableCandidate Create(XmlBindableTarget target) =>
        ValidateTarget(target.TypeSymbol) switch
        {
            { IsSuccess: true, Value: { } typeSymbol } => CreateCandidate(typeSymbol),
            { } failure => new XmlBindableCandidate(null, failure.Diagnostics),
        };

    private static BindingResult<INamedTypeSymbol> ValidateTarget(INamedTypeSymbol typeSymbol) =>
        typeSymbol switch
        {
    ...
    {
        MemberAccumulator accumulator = typeSymbol.GetProperties()
            .Aggregate(
                MemberAccumulator.Empty,
                (state, property) => state.Add(XmlBindableMemberFactory.TryCreate(
    ...
    private static string GetNamespaceName(INamedTypeSymbol typeSymbol) =>
        typeSymbol.ContainingNamespace switch
        {
            { IsGlobalNamespace: false } namespaceSymbol => namespaceSymbol.ToDisplayString(),
            _ => string.Empty,
        };

    private static BindingResult<INamedTypeSymbol> Failure(
        DiagnosticDescriptor descriptor,
        INamedTypeSymbol typeSymbol,
        params object?[] messageArgs) =>
        BindingResult<INamedTypeSymbol>.Failure(Diagnostic.Create(
            descriptor,
