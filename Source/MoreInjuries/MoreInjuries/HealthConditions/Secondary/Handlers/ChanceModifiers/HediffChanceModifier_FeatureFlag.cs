...
public sealed class HediffChanceModifier_FeatureFlag : SecondaryHediffChanceModifier
{
    ...
    public override float GetModifer(Pawn pawn)
    {
        ...
        if (string.IsNullOrEmpty(feature))
        {
            ...
            return BaseChanceOrFail($"{nameof(HediffChanceModifier_FeatureFlag)} is missing the key. Cannot evaluate chance.");
        }
        if (!MoreInjuriesMod.Settings.Keyed.TryGetMember(feature!, out bool flag))
        {
            ...
            return BaseChanceOrFail($"{nameof(HediffChanceModifier_FeatureFlag)}: {feature} is not a valid feature flag in the settings. Cannot evaluate chance.");
        }
        if (!flag)
        {
    ...
    [DoesNotReturn]
#endif
    private static float BaseChanceOrFail(string errorMessage)
    {
